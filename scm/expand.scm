...
(toplevel (write-binary-as-string b . args)
    (let* ((args-len (length args))
           (port (if (> args-len 0) (list-ref args 0) (current-output-port)))
           (start (if (> args-len 1) (list-ref args 1) 0))
           (wrap-char (if (> args-len 2) (list-ref args 2) #\"))
           (escape-code (char->integer wrap-char))
           (end (if (> args-len 3) (list-ref args 3) (binary-length b))))
        (port wrap-char)
        (let loop ((i start))
            (if (< i end)
                (let ((c (binary-ref b i)))
                    (cond ((or (= c escape-code) (= c backslash-code))
                           (port #\\)
                           (port b i (+ i 1)))
                          ((printable? c)
                           (port b i (+ i 1)))
...
(toplevel-helper (display-aux x port is-write)
    (cond ((null? x)
          ...
          ((string? x)
           (if is-write
               (write-binary-as-string x port 1)
               (port x)))
          ((symbol? x)
           (if is-write
               (write-binary-as-string x port 1 #\|)
