...
```bash
./scm/expand < examples/test-loop.scm | ./scm/scan | node --experimental-modules js/mce.mjs
```
...
node --experimental-modules js/mce.mjs < test-loop.cps
...
```bash
$ ./scm/expand < examples/test-state2.scm | ./scm/scan | node --experimental-modules js/mce.mjs | ./scm/mce 
0
1
2
3
